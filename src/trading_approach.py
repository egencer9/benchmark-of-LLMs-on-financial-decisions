...
class TechnicalAnalysisApproach(TradingApproach):
    ...
    def adjust_prompt_instructions(self) -> str:
        return (
            ...
            "POSITION MANAGEMENT:\n"
            "  - If already in a profitable position aligned with both news AND technicals → HOLD (let winners run)\n"
            "  - If in a losing position where news contradicts your direction → FLAT (cut loss)\n"
            "  - If in a position and technicals weaken but news still supports → HOLD (news is primary)\n"
            "  - Avoid flipping LONG↔SHORT on consecutive days — reversals need overwhelming confluence\n\n"
            "CRITICAL RULES:\n"
            ...
            "  - HOLD is a valid, smart decision — especially when technicals are inconclusive (score 0 or ±1). "
            "Do not force a trade just because you have technical data.\n"
